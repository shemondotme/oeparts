...
<x-filament-panels::page>
    <div class="space-y-6" x-data="{ search: '' }">
        <div class="op-card relative overflow-hidden p-6 page-header-gradient page-header-border">
            <div class="absolute inset-0" style="background: radial-gradient(circle at 30% 30%, rgba(99, 102, 241, 0.08), transparent 50%);"></div>
            <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 class="text-lg font-bold tracking-tight flex items-center gap-2" style="font-family: var(--font-display); color: var(--color-text-on-accent);">
                        <x-heroicon-o-wrench-screwdriver class="w-5 h-5" style="color: var(--color-info-500);" />
                        System Management
                    </h2>
                    <p class="mt-1 text-sm max-w-2xl leading-relaxed" style="color: var(--color-text-muted);">
                        Monitor queues, logs, backups, permissions, and scheduled tasks. System-level administration for platform health and security.
                    </p>
                </div>
        ...
        @include('filament.components.cluster-search', ['placeholder' => 'Search system tools... (e.g. backup, cache, logs, jobs)'])

        @php
            $sections = [
                'Monitoring & Health' => [
                    'icon' => 'heroicon-o-heart-pulse',
                    'keywords' => 'health cache redis failed jobs queue',
                    'items' => [
                        ['Health Check', '/admin/health-check', 'System health dashboard with queue, cache, storage, and application checks', 'heroicon-o-heart'],
                        ['Cache Dashboard', '/admin/cache-dashboard', 'Redis memory, hit rate, keys, and cache management', 'heroicon-o-cpu-chip'],
                        ['Failed Jobs', '/admin/failed-jobs', 'View, retry, or clear failed queue jobs', 'heroicon-o-x-circle'],
                    ]
                ],
                'Audit & Logs' => [
                    'icon' => 'heroicon-o-document-magnifying-glass',
                    'keywords' => 'logs activity login search cron',
                    'items' => [
                        ['Log Viewer', '/admin/log-viewer', 'Browse application log files with level filtering', 'heroicon-o-document-text'],
                        ['Scheduled Tasks', '/admin/scheduled-tasks', 'View registered commands and recent cron execution history', 'heroicon-o-clock'],
                    ]
                ],
                'Access & Security' => [
                    'icon' => 'heroicon-o-shield-check',
                    'keywords' => 'permissions roles matrix',
                    'items' => [
                        ['Permission Matrix', '/admin/permission-matrix', 'Visual grid of roles and permissions with toggle', 'heroicon-o-key'],
                    ]
                ],
                'Data & Recovery' => [
                    ...
                    'items' => [
                        ['Backup Management', '/admin/backup-dashboard', 'Create, download, and manage database and configuration backups', 'heroicon-o-arrow-down-tray'],
                    ]
                ],
            ];
        @endphp
                        ...
                        @foreach ($sectionData['items'] as $item)
                            ...
                            >
                                <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg transition-all duration-200 group-hover:scale-110"
                                    ...
                                    @svg($item[3], 'w-5 h-5 transition-colors duration-200', ['style' => 'color: var(--color-brand-600);'])
                                </div>
        ...
        <div x-show="search.length > 0 && !'{{ $sectionKeywords }}'.includes(search.toLowerCase()) && !'{{ $allItemKeywords }}'.includes(search.toLowerCase())"
            ...
            <p class="text-sm font-medium" style="color: var(--color-text-muted);">No system tools match your search.</p>
        </div>
    </div>
</x-filament-panels::page>
