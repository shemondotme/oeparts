...
<div
    x-data="cartData()"
    class="min-h-screen bg-[#F8FAFC] relative overflow-hidden pt-20 pb-28"
>
    {{-- Decorative backgrounds --}}
    <div class="absolute top-0 right-0 w-[40rem] h-[40rem] rounded-full -mr-48 -mt-32 pointer-events-none"
         style="background: radial-gradient(circle, rgba(245,158,11,0.07) 0%, transparent 70%); filter: blur(60px);"></div>
    <div class="absolute bottom-0 left-0 w-[40rem] h-[40rem] rounded-full -ml-64 bottom-10 pointer-events-none"
         style="background: radial-gradient(circle, rgba(59,130,246,0.05) 0%, transparent 70%); filter: blur(60px);"></div>
    ...
    <div x-show="confirmOpen" x-cloak x-trap.noscroll="confirmOpen"
        ...
        <div class="absolute inset-0 bg-navy/40 backdrop-blur-sm" @click="confirmOpen = false"></div>
        <div class="relative bg-white rounded-[28px] shadow-2xl p-8 max-w-sm w-full border border-white z-10"
             x-transition:enter="transition ease-out duration-300"
             x-transition:enter-start="opacity-0 scale-90"
             x-transition:enter-end="opacity-100 scale-100">
            <div class="w-14 h-14 rounded-[18px] bg-red-50 flex items-center justify-center mx-auto mb-5">
                <x-heroicon-o-trash class="w-7 h-7 text-red-500"/>
            </div>
            <h3 id="clear-cart-title" class="font-display text-xl font-black text-navy text-center mb-2">Clear Cart?</h3>
            <p class="text-sm text-muted text-center font-medium mb-8">All items will be removed. This cannot be undone.</p>
            <div class="flex gap-3">
                <button @click="confirmOpen = false"
                        class="flex-1 py-3 rounded-[14px] border-2 border-gray-200 text-navy font-black text-sm hover:border-navy transition-colors">
                    Cancel
                </button>
                <button @click="confirmClearCart()"
                        class="flex-1 py-3 rounded-[14px] bg-red-500 text-white font-black text-sm hover:bg-red-600 shadow-lg shadow-red-500/25 transition-all">
                    Clear Cart
                </button>
            </div>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 md:py-8 relative z-10">
        ...
        <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6"
            ...
            {{-- Step breadcrumb --}}
            <div x-show="cart.items.length > 0" class="flex items-center gap-1 bg-white/60 backdrop-blur-xl rounded-[20px] p-2 border border-white max-w-full overflow-x-auto shadow-sm">
        ...
        >
            ...
            <a href="/{{ app()->getLocale() }}/"
               class="inline-flex items-center gap-3 px-8 py-4 rounded-xl font-black text-white bg-gradient-to-r from-navy to-blue-900 shadow-xl shadow-navy/20 hover:shadow-2xl hover:-translate-y-1 transition-all group">
        ...
        <div x-show="cart.items.length > 0" x-cloak class="grid lg:grid-cols-12 gap-8 lg:gap-10 items-start">
            ...
            <div class="lg:col-span-8 flex flex-col gap-6">
                ...
                <div x-show="errorMessage" x-cloak
                     class="p-4 rounded-[20px] bg-red-50 border border-red-100 flex items-center gap-4 shadow-sm">
                    ...
                    <button @click="errorMessage = ''"
                            class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-red-100 text-red-400 hover:text-red-700 transition-colors">
                        <x-heroicon-o-x-mark class="w-4 h-4"/>
                    </button>
                </div>

                <div class="flex items-center justify-between px-2 mb-1">
                    <h3 class="text-xs font-black uppercase tracking-[0.15em] text-navy">Order Items</h3>
                    <button @click="confirmOpen = true"
                            class="text-[11px] font-black uppercase tracking-[0.1em] text-red-400 hover:text-red-600 flex items-center gap-1.5 transition-colors group">
                ...
                <div class="flex flex-col gap-4">
                    <template x-for="(item, index) in cart.items" :key="item.id">
                        <div class="relative overflow-hidden rounded-[28px] bg-white/90 backdrop-blur-md border border-white p-4 sm:p-5 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-amber/30 hover:shadow-lg"
                            ...
                            <div class="flex flex-col sm:flex-row gap-5 items-center sm:items-stretch">
                                ...
                                <div class="w-20 h-20 sm:w-24 sm:h-24 shrink-0 rounded-[20px] bg-gradient-to-br from-navy to-blue-900 flex items-center justify-center shadow-inner relative group overflow-hidden">
                                    {{-- CSS grid pattern (no external CDN) --}}
                                    <div class="absolute inset-0 opacity-10"
                                         style="background-image: repeating-linear-gradient(0deg,transparent,transparent 3px,rgba(255,255,255,.08) 3px,rgba(255,255,255,.08) 4px),repeating-linear-gradient(90deg,transparent,transparent 3px,rgba(255,255,255,.08) 3px,rgba(255,255,255,.08) 4px);"></div>
                                    ...
                                    <template x-if="item.condition !== 'new'">
                                        <div class="relative z-10">
                                            ...
                                            <div class="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-amber shadow-[0_0_10px_rgba(245,158,11,0.8)] ring-2 ring-navy"></div>
                                        </div>
                                    </template>
                                </div>
                                ...
                                <div class="flex-1 min-w-0 flex flex-col justify-center text-center sm:text-left w-full">
                                    ...
                                    <div class="flex items-center justify-center sm:justify-start gap-3 mt-auto">
                                        ...
                                        <div class="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest"
                                            ...
                                            <x-heroicon-s-check-circle class="w-3.5 h-3.5" x-show="item.in_stock"/>
                                            <x-heroicon-s-x-circle class="w-3.5 h-3.5" x-show="!item.in_stock"/>
                                            <span x-text="item.in_stock ? 'In Stock' : 'Out of Stock'"></span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center shrink-0 w-full sm:w-auto pt-4 sm:pt-0 border-t sm:border-t-0 border-gray-100">
                                    <p class="font-display text-2xl font-black text-navy tracking-tighter leading-none mb-0 sm:mb-4">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="flex items-center bg-gray-50 rounded-[14px] p-1 border border-gray-200 shadow-inner">
                                            <button @click="decrementItem(item.id)"
                                                    ...
                                                    class="w-8 h-8 rounded-[10px] flex items-center justify-center text-navy/50 disabled:opacity-30 border border-transparent transition-all hover:border-gray-300 hover:bg-white hover:shadow-sm active:scale-95">
                                        ...
                                        <button @click="removeItem(item.id)"
                                                ...
                                                class="group w-10 h-10 rounded-[14px] bg-red-50 text-red-400 hover:bg-red-500 hover:text-white transition-all flex items-center justify-center shadow-sm">
                ...
                {{-- Carriers banner --}}
                <div class="mt-4 p-5 sm:p-6 rounded-[28px] bg-white/60 backdrop-blur-xl border border-white shadow-sm flex flex-col gap-5">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h4 class="text-navy font-black text-base">Secure EU Shipping</h4>
                            <p class="text-muted font-medium text-xs mt-0.5">Fast & tracked delivery across all EU member states.</p>
                        </div>
                    </div>
                    <div class="flex flex-wrap gap-3">
                        @foreach([
                            ['name' => 'DHL', 'container' => 'border-red-100 bg-red-50/40 hover:bg-red-50', 'text' => 'text-red-600'],
                            ['name' => 'DPD', 'container' => 'border-blue-100 bg-blue-50/40 hover:bg-blue-50', 'text' => 'text-blue-600'],
                            ['name' => 'GLS', 'container' => 'border-emerald-100 bg-emerald-50/40 hover:bg-emerald-50', 'text' => 'text-emerald-600'],
                        ] as $carrier)
                        <div class="flex items-center gap-3 px-4 py-2.5 rounded-[16px] border shadow-sm transition-colors {{ $carrier['container'] }}">
                            <div class="w-8 h-8 rounded-xl bg-white flex items-center justify-center shadow-inner {{ $carrier['text'] }}">
                                <x-heroicon-o-truck class="w-4 h-4"/>
                            </div>
                            <div class="flex flex-col">
                                <span class="text-sm font-black leading-none mb-0.5 {{ $carrier['text'] }}">{{ $carrier['name'] }}</span>
                                <span class="text-[9px] font-bold text-slate-500 uppercase">Carrier</span>
                            </div>
                        </div>
                        @endforeach
                    </div>
            ...
            <div class="lg:col-span-4 space-y-6 lg:sticky lg:top-28 lg:h-fit">
                ...
                <div class="bg-white/80 backdrop-blur-xl rounded-[36px] p-6 sm:p-8 border border-white shadow-2xl shadow-navy/5 relative overflow-hidden">
                    ...
                    <div class="space-y-4 mb-8">
                        ...
                        <div class="flex justify-between items-center text-sm">
                            <span class="font-bold text-muted">Shipping</span>
                            <span class="text-[10px] uppercase tracking-widest font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-[6px] ring-1 ring-inset ring-emerald-100">Calculated next</span>
                        </div>
                    </div>
                    ...
                    <template x-if="summary.free_shipping_threshold > 0">
                        <div class="mb-8 p-5 rounded-[20px] bg-[#FFF8EB] border border-amber/20 relative overflow-hidden">
                            <div class="flex justify-between items-end mb-3">
                                <span class="text-[10px] font-black uppercase tracking-[0.15em] text-amber">Free Shipping</span>
                                <span class="text-xs font-black text-navy">
                                    ...
                                    <template x-if="summary.shipping_needed <= 0">
                                        <span class="text-emerald-600 flex items-center gap-1">
                                            <x-heroicon-s-check-circle class="w-4 h-4"/> Qualified
                                        </span>
                                    </template>
                                </span>
                            </div>
                            <div class="h-2 w-full bg-amber/10 rounded-full overflow-hidden">
                                <div class="h-full bg-gradient-to-r from-amber to-orange-400 transition-all duration-700 ease-out"
                    ...
                    <template x-if="!summary.coupon_code">
                        <div class="mb-6">
                            <label for="promo_code" class="block text-[10px] font-black uppercase tracking-[0.15em] text-navy mb-2">Promo Code</label>
                            <div class="relative flex items-center group">
                                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                                    <x-heroicon-o-ticket class="w-4 h-4 text-muted group-focus-within:text-amber transition-colors"/>
                                </div>
                                <input type="text" id="promo_code" x-model="couponCode" placeholder="Enter discount code"
                                       class="block w-full pl-10 pr-24 py-3 bg-gray-50 border border-gray-200 rounded-[14px] text-sm font-mono uppercase text-navy placeholder:text-gray-400 placeholder:normal-case focus:ring-2 focus:ring-amber/30 focus:border-amber transition-all">
                                <button @click="applyCoupon()" :disabled="!couponCode"
                                        class="absolute right-1.5 inset-y-1.5 px-4 rounded-[10px] bg-navy text-white text-[10px] font-black uppercase tracking-widest hover:bg-blue-900 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                                    Apply
                                </button>
                            </div>
                    ...
                    <div class="pt-6 border-t border-gray-100 mb-8">
                        <div class="flex justify-between items-center gap-4">
                            <div>
                                <span class="text-[11px] font-black uppercase tracking-[0.2em] text-muted block mb-0.5">Total Amount</span>
                                <span class="text-[9px] font-bold uppercase text-emerald-600 tracking-wider">Including all taxes</span>
                            </div>
                            <p class="font-display text-4xl sm:text-5xl font-black text-navy tracking-tighter leading-none">
                    ...
                    <a href="/{{ app()->getLocale() }}/checkout"
                       class="flex items-center justify-between w-full h-16 px-8 rounded-[20px] bg-gradient-to-r from-amber via-orange-400 to-amber bg-[length:200%_auto] text-navy font-black text-sm uppercase tracking-[0.1em] shadow-xl shadow-amber/30 hover:shadow-2xl hover:shadow-amber/40 hover:-translate-y-1 hover:bg-[position:right_center] transition-all duration-300 group">
                        ...
                        <div class="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center group-hover:scale-110 transition-transform">
                            <x-heroicon-o-arrow-right class="w-4 h-4 text-navy group-hover:translate-x-0.5 transition-transform"/>
                        </div>
                    </a>

                    {{-- Trust badges --}}
                    <div class="mt-6 pt-4 border-t border-gray-100 flex items-center justify-center gap-6">
                        <div class="flex items-center gap-1.5 opacity-60 hover:opacity-100 transition-opacity">
                            ...
                            <span class="text-[10px] font-bold text-navy uppercase tracking-widest">SSL Secure</span>
                        </div>
                        <div class="flex items-center gap-1.5 opacity-60 hover:opacity-100 transition-opacity">
                            <x-heroicon-s-credit-card class="w-4 h-4 text-navy/60"/>
                            <span class="text-[10px] font-bold text-navy uppercase tracking-widest">Airwallex</span>
                        </div>
                    </div>
                </div>

                {{-- Support card --}}
                <div class="bg-white/60 backdrop-blur-xl rounded-[28px] p-6 border border-white shadow-lg shadow-navy/5 relative overflow-hidden hover:bg-white/80 transition-colors">
                    <div class="flex items-start gap-4">
                        <div class="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center shrink-0">
                            <x-heroicon-o-chat-bubble-left-ellipsis class="w-5 h-5 text-blue-500"/>
                        </div>
                        <div>
                            <h5 class="text-xs font-black text-navy uppercase tracking-[0.1em] mb-1.5">Need help?</h5>
                            <p class="text-xs text-muted mb-4 font-medium leading-relaxed">Mon–Fri 09:00–18:00. Our support team is ready to assist.</p>
                            <a href="/{{ app()->getLocale() }}/contact"
                               class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-navy to-blue-900 text-xs font-bold text-white hover:shadow-lg hover:shadow-navy/20 transition-all group">
    ...
    <div x-show="cart.items.length > 0" x-cloak
        ...
        <div class="flex items-center justify-between gap-6 max-w-lg mx-auto">
            <div>
                <p class="text-[10px] font-black uppercase tracking-[0.15em] text-muted mb-0.5">Total Amount</p>
                <p class="text-2xl font-black text-navy tracking-tight leading-none">€<span x-text="summary.grand_total.toFixed(2)"></span></p>
            </div>
            <a href="/{{ app()->getLocale() }}/checkout"
               class="flex-1 flex items-center justify-center gap-2 h-14 rounded-2xl bg-gradient-to-r from-amber to-orange-500 text-navy text-sm font-black uppercase tracking-[0.1em] shadow-lg shadow-amber/30 hover:shadow-xl active:scale-95 transition-all">
                Checkout
