...
<div class="min-h-screen bg-[#F8FAFC] pb-24">
    ...
    <tbody
      class="border-b border-gray-100 transition-colors {{ $index % 2 === 1 ? 'bg-navy/[0.02]' : 'bg-transparent' }} hover:bg-gray-50/50"
      x-data="tableRow({ productId: {{ $product->id }}, oemNumber: '{{ $product->oem_number }}', manufacturerName: '{{ $manufacturer ? trans_field($manufacturer->name) : __('search.unknown_brand') }}' })"
    >
                                {{-- Main product row --}}
                                <tr
                                ...
                                <tr class="table-row">
                                    <td colspan="5" class="py-16">
                                        <div class="flex flex-col items-center justify-center text-center max-w-md mx-auto">
                                            <div class="w-14 h-14 rounded-xl bg-amber/15 border border-amber/25 flex items-center justify-center mb-5">
                                                <x-heroicon-o-magnifying-glass class="w-7 h-7 text-amber-text" />
                                            </div>
                                            <h3 class="font-display font-bold text-xl text-navy mb-2">{{ __('search.no_results') }}</h3>
                                            <p class="text-muted text-sm mb-6 leading-relaxed">
                                                We couldn't find any direct match for <strong class="text-navy font-mono">{{ $normalized_query }}</strong>. Our supplier network likely has it.
                                            </p>
                                            <button type="button"
                                                    onclick="window.dispatchEvent(new CustomEvent('open-inquiry-modal'))"
                                                    class="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-amber text-navy font-bold text-sm hover:bg-amber/90 transition-colors">
                                                <x-heroicon-o-paper-airplane class="w-4 h-4" />
                                                Submit Free Part Inquiry
                                            </button>
                                        </div>
                                    </td>
                                </tr>
...
<div
      x-data="cardRow({
        ...
        manufacturerName: '{{ $manufacturer ? trans_field($manufacturer->name) : __('search.unknown_brand') }}'
                ...
                @empty
                <div class="py-12 px-6 bg-white rounded-xl border border-gray-100 shadow-sm mt-4 text-center">
                    <div class="flex flex-col items-center max-w-sm mx-auto">
                        <div class="w-12 h-12 rounded-xl bg-amber/15 border border-amber/25 flex items-center justify-center mb-4">
                            <x-heroicon-o-magnifying-glass class="w-6 h-6 text-amber-text" />
                        </div>
                        <h3 class="font-display font-bold text-lg text-navy mb-2">{{ __('search.no_results') }}</h3>
                        <p class="text-muted text-xs mb-5 leading-relaxed">
                            We couldn't find <strong class="text-navy font-mono">{{ $normalized_query }}</strong>. Ask our suppliers directly.
                        </p>
                        <button type="button"
                                onclick="window.dispatchEvent(new CustomEvent('open-inquiry-modal'))"
                                class="w-full inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-amber text-navy font-bold text-sm hover:bg-amber/90 transition-colors">
                            <x-heroicon-o-paper-airplane class="w-4 h-4" />
                            Submit Free Part Inquiry
                        </button>
                    </div>
...
// Card view Alpine component
function cardRow(config) {
    return {
        shown: false,
        detailsOpen: false,
        crossRefsOpen: false,
