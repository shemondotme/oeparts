    ...
    <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-gray-600 mt-1">{{ trans_field($faq->question) }}</p>
            </div>
        </div>
    </div>
            ...
            <form action="{{ route('admin.cms.faqs.update', $faq) }}" method="POST">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <label for="is_active" class="text-sm text-gray-700">FAQ is active</label>
                    </div>
                    ...
                    <div x-data="{ lang: 'en' }">
                        ...
                        @foreach(['en','de','lt','fr','es'] as $lang)
                            <input type="text"
                                   name="question[{{ $lang }}]"
                                   x-show="lang === '{{ $lang }}'"
                                   value="{{ old('question.'.$lang, $faq->question[$lang] ?? '') }}"
                    ...
                    <div x-data="{ lang: 'en' }">
                        ...
                        @foreach(['en','de','lt','fr','es'] as $lang)
                            <textarea name="answer[{{ $lang }}]"
                                      ...
                                      {{ $lang === 'en' ? 'required' : '' }}
                                      class="w-full rounded-lg border-gray-300 text-sm focus:ring-amber-500 focus:border-amber-500">{{ old('answer.'.$lang, $faq->answer[$lang] ?? '') }}</textarea>
                        @endforeach
                    </div>
                </div>
