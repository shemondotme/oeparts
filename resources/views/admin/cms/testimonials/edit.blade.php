...
<div class="px-6 py-8">
    <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-gray-600 mt-1">{{ $testimonial->author_name }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-3xl">
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <form action="{{ route('admin.cms.testimonials.update', $testimonial) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div x-data="{ lang: 'en' }">
                        ...
                        @foreach(['en','de','lt','fr','es'] as $lang)
                            <textarea name="content[{{ $lang }}]"
                                      ...
                                      maxlength="1000"
                                      {{ $lang === 'en' ? 'required' : '' }}
                                      class="w-full rounded-lg border-gray-300 text-sm focus:ring-amber-500 focus:border-amber-500">{{ old('content.'.$lang, $testimonial->content[$lang] ?? '') }}</textarea>
                        @endforeach
                    </div>
