...
<div class="max-w-4xl space-y-6">
    ...
    <section class="bp-card overflow-hidden">
        <header class="bp-card-header">
            <p class="bp-spec text-amber-ink">§ Catalog · New Product</p>
            <h2 class="mt-1 font-display text-xl font-bold text-ink tracking-[-0.02em]">
                Add Product<span class="text-amber">.</span>
            </h2>
        </header>
        ...
        <form action="{{ route('admin.catalog.products.store') }}" method="POST">
                    ...
                    <div x-show="activeTab === '{{ $code }}'" class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="description[{{ $code }}]" rows="4"
                                      ...
                                      class="bp-input w-full resize-y">{{ old('description.' . $code) }}</textarea>
            ...
            <div class="px-5 py-4 bg-ivory-alt border-t border-rule flex items-center justify-end gap-3">
                ...
                <button type="submit" class="bp-btn-primary">Create Product</button>
            </div>
        </form>
    </section>
