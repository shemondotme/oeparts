...
### Added
- **Release signature verification** (Update System **Chunk 6.1**, Phase 6 / V2 hardening begins) — releases can now be cryptographically signed (RSA-SHA256) and verified against an app-baked public key, so the updater authenticates *who* produced a release rather than only that the download is intact. The release pipeline signs each build with a CI-secret private key and carries the signature in `version.json` and the release catalog; the update pre-flight blocks any release whose signature is missing or invalid once a public key is provisioned (opt-in until then). This defends against a tampered manifest or a swapped zip that still checksums cleanly. 6 new tests.
- **Update System end-to-end CI** (Update System **Chunk 5.3** — completes Phase 5 and the V1 Update & Recovery System) — a capstone end-to-end test builds a real release zip, serves it through a faked endpoint, and drives the real download → extract → swap → rollback pipeline, then runs the whole apply state machine with a real pre-update backup and a forced post-swap failure to prove the failure/rollback matrix reverses both files and the database. The CI test matrix now covers PHP 8.2/8.3/8.4 with a fresh-migration check. 2 new tests. **This ships the complete one-click in-app Update & Recovery System: update checker, backup engine, one-click update engine, app-independent recovery console, and the release/CI pipeline.**
- **Release manifest generation + GitHub Release automation** (Update System **Chunk 5.2**) — the `oeparts:release:manifest` command (backed by `ReleasePublisher`) folds a built zip's SHA-256, size, and versioned download URL into `version.json` and upserts the `releases.json` catalog the in-app updater reads (newest-first, `latest` tracked, idempotent per version). A tag-triggered GitHub Actions workflow builds the zip, generates the manifests, publishes them to `main`, and attaches the zip plus both JSON files to a GitHub Release — replacing the earlier ad-hoc in-place zip. 6 new tests.
