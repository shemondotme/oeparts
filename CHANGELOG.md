...
- Update from **Settings → System Updates**, exactly as before. The update runs **21 new database migrations**; all of them (and the older ones that were hardened) are idempotent, so a database left half-migrated by an earlier interrupted update attempt is handled instead of failing on "already exists". On a catalog of around a million products the step that adds this release's new columns to the `products` table rebuilds that table and its full-text index **once** (several minutes; it used to be five separate rebuilds), and the site stays in maintenance mode meanwhile — schedule the update for a quiet hour.
- Once the progress bar reaches "Running migrations", the System Updates page you started the update from may stop updating (the new release cannot talk to a page rendered by the old one). That is expected: the update keeps running on the server and finishes on its own — on a very large catalog the migration step takes several minutes. Reload System Updates afterwards to see the result.
- The update signals your queue workers to restart (`queue:restart`), because a long-running worker keeps the old code in memory. Afterwards, confirm your process manager (supervisor/systemd) has brought the worker back up.
...
- **An update could stall right after the files were swapped, leaving the site in maintenance mode.** The update is driven by the admin's browser tab, and that tab was rendered by the *old* release. Once the new code lands, the tab's Livewire session is rejected (the new Livewire version answers every further poll with "419 page expired"), and a freshly loaded admin page used to answer 500 until the new migrations had run — so nothing ever ran the migration step. The release that has just been installed now finishes its own update: the first request that reaches it (the tab's next poll, an admin reload, a visitor being shown the maintenance page, or the hourly watchdog) runs the remaining steps exactly as the tab would have. Found by rehearsing the real 1.0.16 → 2.0.0 self-update.
- On a very large catalog the migration step could be started a **second time on top of itself**. Each update step holds a lock so overlapping polls cannot run it twice, but the lock expired after five minutes while a single migration step (every `ALTER TABLE products` rebuilds the table and its full-text index) takes far longer at a million products — so the next poll started a duplicate `migrate`, queuing identical `ALTER TABLE`s behind each other's locks and writing duplicate rows into `migrations`. The lock now outlasts the slowest step (1 hour, `OE_UPDATE_ADVANCE_LOCK`).
- The admin "Review & apply update" panel never showed a release's **breaking changes or pre-update notes** — the update catalog dropped both fields, so the panel received neither. They are now carried into the catalog and shown before you click Apply.
- Static files added to `public/` after your installed release (for example the product-image placeholder) are now delivered by the update. A self-update only replaces the paths its installed release lists, so such a file was silently missing on updated sites, and every product without a photo pointed at a 404. A test now fails whenever a new `public/` file is neither republished nor consciously exempt.
- Admin sidebar badges no longer take the whole admin panel down when a table does not exist yet (the window between the file swap and the migrations).
- The new https/canonical-host redirect no longer sends every request in an endless loop on sites where a proxy terminates TLS and speaks plain http to the app (Cloudflare "Flexible" SSL, some load balancers).
...
#### Fixed — Platform
- The web installer no longer starts a step a second time while the first is still running. Its progress page re-polls after any failed response, so on a server whose timeout is shorter than the schema-creation step (~150 migrations), the retry started a second `migrate:fresh` on top of the first and dropped tables out from under it. An overlapping poll now just reports progress.
...
### Fixed — Update & Backup Engine
- **Backups could silently include (or crash trying to include) a completely different database's tables** — on any MySQL server where the app's database user also has access to another database (common: one MySQL user shared across multiple sites, or a local dev machine with several local installs), the backup engine's table listing wasn't scoped to the app's own database, so it could pull in a sibling database's tables too. Found and fixed via a real end-to-end update rehearsal against a genuine multi-database MySQL server, where it crashed the pre-update backup outright.
- **A backup or update that failed at exactly the wrong moment could leave the app permanently locked out of every future backup and update** — if turning maintenance mode back off (itself a database write) failed while cleaning up after a failure, the lock-release step that should always run next never got the chance, and recovering required manually deleting a lock file by hand. All three cleanup paths (success, failure, and a failure during start-up itself) now release the lock unconditionally, and a start-up failure no longer has its real error message masked by a secondary cleanup error.
- **The scheduled daily update check could crash outright if the mail server wasn't configured or was temporarily down** — notifying admins of a new version is a side effect of a successful check, not the check itself; a failed notification is now logged and safely retried on the next run instead of taking the whole command down with it.
