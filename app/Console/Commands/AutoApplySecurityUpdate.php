 ...
 * mirrors WordPress's automatic background updates for security releases,
 * which this project deliberately scoped to security-only (never routine
 * feature releases, which may carry breaking changes an operator should
 * review). Reuses UpdateApplier::start()/run() — the EXACT same chunked,
 * pre-flight-gated, backup-first, auto-rollback-on-failure FSM the admin
 * dashboard's "Apply Update" button drives (App\Filament\Pages\System\
 ...
 * emailing super_admins the outcome (WordPress's own silent background
 * updates are a common complaint — this is deliberately loud instead).
 */
