...
/**
 * WP-Cron-style fallback: if the site's real system cron was never configured
 * (or stopped running), overdue scheduled tasks (backups, sitemap, abandoned-
 * cart emails, update checks, …) would otherwise just silently never fire —
 * the only signal today is an admin happening to notice the dashboard's
 * "Scheduler: stale" tile. This middleware triggers `schedule:run` from a
 * normal HTTP request's terminate() phase instead — the same trick
 * WordPress's wp-cron.php uses, adapted to run in-process rather than a
 * loopback HTTP request (simpler, no outbound network call needed, matches
 * rule #41's pure-PHP philosophy).
 *
 ...
 * this fires — an accepted trade-off, same one WordPress makes.
 *
